...
export const sendFriendRequest = async (req, res) => {
  try {
    ...
    // Gửi thông báo realtime tới người nhận (nếu đang online)
    const populatedRequest = await FriendRequest.findById(request._id).populate(
      "from",
      "_id username displayName avatarUrl"
    );

    io.to(to.toString()).emit("friend-request", populatedRequest);
...
export const acceptFriendRequest = async (req, res) => {
  try {
    ...
    // Báo realtime cho người đã gửi lời mời biết họ vừa được chấp nhận kết bạn
...
export const removeFriend = async (req, res) => {
  try {
    ...
    // báo realtime cho người kia để danh sách bạn bè của họ cũng cập nhật
    io.to(friendId.toString()).emit("friend-removed", {
      userId: userId.toString(),
    });

    return res.status(200).json({ message: "Đã xoá bạn bè" });
  } catch (error) {
